...
- ***"scripts_etc"*:** The scripts that are used for result analyses and data visualization are located in the subdirectories under this directory.
  ...
  - *"shell_scrpits"***:** The Unix shell scripts used for HADDOCK2.2 result analyses and input structure manipulation are located in the subdirectories under this directory.
    ...
    - *"HADDOCK_output_analysis"***:** HADDOCK calculates the intermolecular interactions using its scoring function "_total score_", and scoring terms such as van der Waals energy, electrostatic energy, and desolvation energy. In addition, HADDOCK calculates the buried surface area of the complexes. Each coordinate file generated by HADDOCK starts with a header section, in which the above mentioned scores are embedded. The scripts in this directory allows easy extraction of the best scores (highest predicted affinities) from multiple HADDOCK run outputs, by ranking the different scores separately.

### Usage
The scripts in this repository and the commands given below works in Unix environment. For Windows, Unix terminal can be installed via Windows Store application. Despite there may be stability issues, Unix terminal in Windows also can be used for executing the shell scripts and the commands explained below.
...
**5)** By using the command given below, make the script files executable:
```
chmod +x filename
```
e.g: chmod +x get_BSA
